...
ipcMain.on('ipc-example', async (event, arg) => {
  ...
  event.reply('ipc-example', msgTemplate('pong sya'));
});
...
ipcMain.on('show-save-dialog', (event, arg) => {
  ...
  dialog
    ...
    .then((result) => {
      ...
      const ourData = arg.theData.map((friend, index) => ({
        no: index,
      ...
      utils.sheet_add_json(ws, ourData, { origin: 'A2', skipHeader: true });
      utils.book_append_sheet(wb, ws, 'Responses');
      const content = write(wb, {
        type: 'buffer',
        bookType: 'xlsx',
        bookSST: false,
      });
      fs.writeFileSync(`${result.filePath}.xlsx`, content);
    })
    .catch((...args) => {
